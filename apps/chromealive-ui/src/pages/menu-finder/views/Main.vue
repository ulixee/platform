<template>
  <div class="Finder absolute">
    <div v-if="isSelectMode" class="waiting-for-selection">
      WAITING FOR ELEMENT
    </div>
    <div v-else class="overlay-box">
      <div v-if="selectedElement" class="element-view">
        <div class="header-bar flex flex-row">
          <div @click="backToMain">
            <ChevronLeftIcon class="w-10" />
          </div>
          <div
            class="flex-1 border-t border-gray-400"
            @mouseenter="highlightNode(selectedElement.backendNodeId)"
            @mouseleave="hideHighlight"
          >
            &lt;{{ selectedElement.localName }}&gt;{{
              selectedElement.nodeValueInternal || (selectedElement.hasChildren ? '...' : '')
            }}&lt;/{{ selectedElement.localName }}&gt;
          </div>
          <img
      ...
      <div v-else class="search-view">
        ...
        <div class="results p-5">
          ...
          <ul v-if="results">
            <li
              v-for="record of results"
              class="border-t border-gray-400"
              @click="selectElement(record)"
              @mouseenter="highlightNode(record.backendNodeId)"
              @mouseleave="hideHighlight"
            >
              &lt;{{ record.localName }}&gt;{{
                record.nodeValueInternal || (record.hasChildren ? '...' : '')
              }}&lt;/{{ record.localName }}&gt;
            </li>
          </ul>
        </div>
        ...
        <div
          ...
          @mouseenter="highlightNode(devtoolsElement?.backendNodeId)"
          @mouseleave="hideHighlight"
        >
          {{ tagPreview }}
        </div>
...
import { CheckIcon, SelectorIcon, ChevronLeftIcon } from '@heroicons/vue/solid';
...
export default Vue.defineComponent({
  ...
  mounted() {
    window.addEventListener('focus', () => this.inputElem.focus());

...
> extends Function<TSchema, TPlugin1, TPlugin2, TPlugin3, TContext> {
  ...
  public cacheHasJsonInput = false;
  public cache?: Table<
    TSchema['input'] & {
  ...
  public override attachToDatabox(
    databoxInternal: DataboxInternal<any, any>,
    functionName: string,
  ): void {
    super.attachToDatabox(databoxInternal, functionName);
    if (!this.crawlerComponents.disableCache) {
      let inputSchema = this.crawlerComponents.schema?.input;
      if (!inputSchema || !Object.keys(inputSchema).length) {
        this.cacheHasJsonInput = true;
        inputSchema = { input: string() } as any;
      }
      ...
      databoxInternal.attachTable(this.cache);
    }
  }
  ...
  protected async runWrapper(
    ...
    const { outputs, Output, databoxMetadata, input, schema, ...rest } =
  ...
  protected async saveToCache(
    ...
    const inputFields = this.getInputFields(input);
  ...
  protected async findCached(input: TContext['input']): Promise<ICrawlerOutputSchema> {
    ...
    const extraCols = this.getInputFields(input);
    const extraSql =
    ...
    const cached = await this.cache.query(
      `select * from self where runTime >= $1 and ${extraSql} limit 1`,
      [maxAge, ...Object.values(extraCols)],
    );
  ...
  protected getInputFields(input: TContext['input']): Record<string, any> {
    const { maxTimeInCache, sessionId, ...inputArgs } = input;
    const extraCols: { [key: string]: any } = {};
    if (Object.keys(inputArgs).length) {
      if (this.cacheHasJsonInput) {
        extraCols.input = TypeSerializer.stringify(inputArgs, { sortKeys: true });
      } else {
        for (const [key, value] of Object.entries(inputArgs)) {
          const fieldSchema = this.crawlerComponents.schema.input[key];
          if (fieldSchema) {
            extraCols[key] = SqlGenerator.convertToSqliteValue(
              this.cache.schema[key].typeName,
              value,
            )[0];
          }
        }

...
- overrides `object`. Setting any of the above properties into the overrides field will override their settings as the manifest is built. For instance, to reset the version history, you can add the property `linkedVersions: []` to `overrides`.
...
- `overrides`: an object list of any overrides you wish to apply to the manifest.

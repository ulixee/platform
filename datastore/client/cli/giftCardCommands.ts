...
export default function giftCardCommands(): Command {
  ...
  cli
    .command('create')
    .description('Create a gift card for a datastore.')
    .argument('<path>', 'The path of the entrypoint to the datastore.')
    ...
    .action(async (path, { identity, amount, host, minerDbHost }) => {
      ...
      if (identity) {
        ...
        if (!signer && specifiedIdentity)
          ...
          throw new Error(
            `The gift card issuer for this Datastore could not be found in the gift card issuers directory (${Path.join(
              giftCardIssuersDir,
              `${identity}.pem`,
            )}).`,
          );
...
!! You must re-upload your Datastore for gift cards to take effect. !!
...
`);
      ...
      if (minerDbHost) {
        console.log(`Or they can try out your datastore over a PostgreSQL connection:

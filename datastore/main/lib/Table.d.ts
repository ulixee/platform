...
export declare type IExpandedTableSchema<T> = T extends Record<string, ISchemaAny> ? {
    [K in keyof T]: T[K];
} : never;
export default class Table<TSchema extends IExpandedTableSchema<any> = IExpandedTableSchema<any>, TSchemaType extends ExtractSchemaType<TSchema> = ExtractSchemaType<TSchema>> {
    ...
    fetchInternal(options?: TQueryCallMeta & {
        input: TSchemaType;
    }): Promise<TSchemaType[]>;
    insertInternal(...records: TSchemaType[]): Promise<void>;
    queryInternal<T = TSchemaType[]>(sql: string, boundValues?: any[], options?: TQueryCallMeta): Promise<T>;

...
class DatastoreApiClient {
    ...
    async registerEscrow(datastoreId, balanceChange) {
        const result = await this.runApi('Escrow.register', {
            escrow: balanceChange,
            datastoreId,
        });
    ...
    async runApi(command, args, timeoutMs) {
        try {
            if (this.validateApiParameters) {
                const schema = datastore_1.default[command] ?? EscrowApis_1.EscrowApisSchema[command];
    ...
    static async lookupDatastoreHost(datastoreUrl, mainchainUrl) {
        const mainchainClient = mainchainUrl ? await localchain_1.MainchainClient.connect(mainchainUrl, 10e3) : null;
        const lookup = await new DatastoreLookup_1.default(mainchainClient).getHostInfo(datastoreUrl);
        await mainchainClient?.close();
        return lookup;
    }
    static createExecSignatureMessage(payment, nonce) {
        return (0, hashUtils_1.sha256)((0, bufferUtils_1.concatAsBuffer)('Datastore.exec', payment?.credits?.id, payment?.escrow?.id, nonce));
    }

...
class SqliteStorageEngine extends AbstractStorageEngine_1.default {
    ...
    async close() {
        await __classPrivateFieldGet(this, _SqliteStorageEngine_db, "f").close();
    }
    query(sql, boundValues, metadata, virtualEntitiesByName) {
        ...
        if (sqlParser.isInsert() || sqlParser.isDelete() || sqlParser.isUpdate()) {
            if (sqlParser.hasReturn() === true) {
                return __classPrivateFieldGet(this, _SqliteStorageEngine_db, "f").prepare(parsedSql).get(valueMap);
            }

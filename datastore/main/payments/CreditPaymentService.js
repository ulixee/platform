...
class CreditPaymentService extends eventUtils_1.TypedEventEmitter {
    get datastoreId() {
    ...
    static async loadAll(fromDir = CreditPaymentService.defaultBasePath) {
    ...
    static async load(datastoreId, creditId, fromDir = CreditPaymentService.defaultBasePath) {
        const storePath = Path.join(fromDir, `${datastoreId}-${creditId}.json`);
        const credit = await (0, fileUtils_1.readFileAsJson)(storePath);
        return new CreditPaymentService(credit, fromDir);
    }
    static async storeCredit(datastoreId, datastoreVersion, host, credits, creditsDir = CreditPaymentService.defaultBasePath) {
    ...
    static async lookup(datastoreUrl, credit, datastoreLookup, creditsDir = CreditPaymentService.defaultBasePath) {
...
CreditPaymentService.MIN_BALANCE = 1;
CreditPaymentService.defaultBasePath = Path.join((0, dirUtils_1.getDataDirectory)(), 'ulixee', 'credits');
exports.default = CreditPaymentService;

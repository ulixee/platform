...
async function deploy(entrypoint, options) {
    ...
    const result = await upload(dbx, packager.manifest, options.cloudHost, options.clearVersionHistory, options.identityPath, options.identityPassphrase);
...
async function upload(dbx, manifest, cloudHost, clearVersionHistory, identityPath, identityPassphrase) {

    ...
    async lookupRemoteDatastoreExtractorPricing(meta, extractor) {
        ...
        try {
            ...
            if (!upstreamMeta.extractorsByName[extractorName]) {
                throw new Error(`${extractorName} is not a valid extractor for ${datastoreId}@v${datastoreVersion}. Valid names are: ${Object.keys(upstreamMeta.extractorsByName).join(', ')}.`);
            }
    ...
    async lookupRemoteDatastoreCrawlerPricing(meta, crawler) {
        ...
        try {
            ...
            if (!upstreamMeta.crawlersByName[crawlerName]) {
                throw new Error(`${crawlerName} is not a valid crawler for ${datastoreId}@v${datastoreVersion} Valid names are: ${Object.keys(upstreamMeta.crawlersByName).join(', ')}.`);
            }
    ...
    async lookupRemoteDatastoreTablePricing(meta, table) {
        ...
        try {
            ...
            if (!upstreamMeta.tablesByName[tableName]) {
                throw new Error(`${tableName} is not a valid table for ${datastoreId}@v${datastoreVersion}.  Valid names are: ${Object.keys(upstreamMeta.tablesByName).join(', ')}.`);
            }

...
class DatastoreRegistry extends TypedEventEmitter_1.default {
    constructor(datastoreDir, apiClients, connectionToHostedServiceCore, config, installCallbackFn) {
        super();
        this.config = config;
        this.installCallbackFn = installCallbackFn;
        this.networkCacheTimeMins = 48 * 60; // 48 hours
        this.stores = [];
    ...
    async get(id, version, throwIfNotExists = true) {
        ...
        if (throwIfNotExists) {
            throw new errors_1.DatastoreNotFoundError(`Datastore version (${version}) not found on Cloud.`, {
                latestVersion,
                version,
            });
        }

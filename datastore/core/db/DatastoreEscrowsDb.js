...
class DatastoreEscrowsDb {
    constructor(baseDir, datastoreId) {
        this.datastoreId = datastoreId;
        this.paymentIdByEscrowId = new Map();
    ...
    `);
        this.insertStatement = this.db.prepare('INSERT INTO escrows (id, allocated, remaining, expirationDate) VALUES (:id, :microgons, :microgons, :expirationDate)');
    ...
    create(id, allocatedMilligons, expirationDate) {
        ...
        if (!result.changes || result.changes < 1)
            throw new Error('Could not create the escrow.');
        return {
            id,
            allocated: microgons,
            remaining: microgons,
    ...
    get(id) {
        ...
        if (!record)
            throw new Error('No PaymentChannelEscrow found');
    ...
    debit(queryId, payment) {
        ...
        if (!escrowId)
            throw new Error('No escrow payment provided. Internal code issue.');
        ...
        if (!result.changes || result.changes < 1) {
            ...
            if (Math.ceil((escrow.allocated - escrow.remaining + payment.microgons) / 1000) >
                Number(payment.escrow.settledMilligons)) {
                throw new Error(`This escrow needs a larger settlement to debit. Current settledMilligons=${payment.escrow.settledMilligons}, New spentMicrogons=${escrow.allocated - escrow.remaining + payment.microgons} (ceiling to nearest milligon)`);
            }
            throw new Error('Could not debit the escrow.');
        }
        return { shouldFinalize: true };
    }
    finalize(escrowId, uuid, finalMicrogons) {
        const entry = this.paymentIdByEscrowId.get(escrowId)?.get(uuid);
        if (!entry)
            throw new Error('Could not find the initial payment for the given Escrow.');
        if (finalMicrogons < 0)
...
exports.default = DatastoreEscrowsDb;
//# sourceMappingURL=DatastoreEscrowsDb.js.map
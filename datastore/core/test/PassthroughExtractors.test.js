...
export default new Datastore({
  ...
  payment: {
    notaryId: 1,
    address: "${new keyring_1.Keyring().createFromUri('upstream').address}",
  },
  extractors: {
    pass: new Datastore.PassthroughExtractor({
      upcharge: 400,
...
export default new Datastore({
  ...
  extractors: {
    ...
    const corePaymentService = new _MockPaymentService_1.default(client2, cloudNode.datastoreCore.datastoreApiClients.get(await cloudNode.address));
    const escrows = [];
    escrowSpendTrackerMock.mock((id, balanceChange) => {
        const escrowId = (paymentService.paymentsByDatastoreId[id] ?? corePaymentService.paymentsByDatastoreId[id]).escrowId;

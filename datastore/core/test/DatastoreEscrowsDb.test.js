...
const db = new DatastoreEscrowsDb_1.default(storageDir, 'test1');
...
test('handles the flow of a payment', async () => {
    ...
    const result = db.debit('q1', {
        uuid: 'p1',
        microgons: 100,
        escrow: { id: 'test1', settledMilligons: 1n, settledSignature: Buffer.from((0, hashUtils_1.sha256)('123')) },
    });
...
test('rejects payments on expired escrows', async () => {
    ...
    expect(() => {
        db.debit('q2', {
            uuid: 'p2',
            microgons: 100,
            escrow: { id: 'test2', settledMilligons: 1n, settledSignature: Buffer.from((0, hashUtils_1.sha256)('123')) },
        });
...
test('rejects a payment with too small a settlement signature', async () => {
    ...
    expect(() => {
        db.debit('q3', {
            uuid: 'p3',
            microgons: 2000,
            escrow: { id: 'test3', settledMilligons: 1n, settledSignature: Buffer.from((0, hashUtils_1.sha256)('123')) },
        });
    }).toThrow(/settlement/);
});
test('rejects a payment with too small a settlement signature within a milligon', async () => {
    ...
    db.debit('q4', {
        uuid: 'p4',
        microgons: 1900,
        escrow: { id: 'test4', settledMilligons: 2n, settledSignature: Buffer.from((0, hashUtils_1.sha256)('123')) },
    });
    db.debit('q6', {
        uuid: 'p6',
        microgons: 99,
...
//# sourceMappingURL=DatastoreEscrowsDb.test.js.map
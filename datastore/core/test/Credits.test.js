...
beforeAll(async () => {
    ...
    CreditPaymentService_1.default.defaultBasePath = Path.join(storageDir, `credits.json`);
...
beforeEach(() => {
    storageCounter += 1;
    LocalchainPaymentService_1.default.storePath = Path.join(storageDir, `payments-${storageCounter}.json`);
    CreditPaymentService_1.default.defaultBasePath = Path.join(storageDir, `credits-${storageCounter}`);
...
test('should be able run a Datastore with Credits', async () => {
    ...
    await CreditPaymentService_1.default.storeCredit(manifest.id, manifest.version, client.connectionToCore.transport.host, credits);
    const paymentService = new LocalPaymentService_1.default();
...
test('should remove an empty Credits from the local cache', async () => {
    ...
    const paymentService = await CreditPaymentService_1.default.storeCredit(manifest.id, manifest.version, client.connectionToCore.transport.host, credits);
...
test('should be able to embed Credits in a Datastore', async () => {
    ...
    }));
    {
        ...
        const credit2Service = await CreditPaymentService_1.default.storeCredit(manifest2.id, manifest2.version, client.connectionToCore.transport.host, credits2);

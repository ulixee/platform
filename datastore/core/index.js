    ...
    async start(options) {
        ...
        try {
            ...
            if (paymentServiceConnection) {
                const argonReserver = new RemoteReserver_1.default(paymentServiceConnection);
                ...
                this.upstreamDatastorePaymentService = new EmbeddedPaymentService_1.default();
                if (!this.datastoreHostLookup) {
                    const mainchainUrl = this.options.localchainConfig?.mainchainUrl ?? env_1.default.localchainConfig?.mainchainUrl;
                    const mainchainClient = mainchainUrl
                        ? await localchain_1.MainchainClient.connect(mainchainUrl, 10e3)
                        : null;

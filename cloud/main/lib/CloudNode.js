...
const kad_1 = require("@ulixee/kad");
...
class CloudNode {
    constructor(config = { shouldShutdownOnSignals: true }) {
        ...
        this.cloudConfiguration = {
            ...
            cloudType: env_1.default.cloudType,
            kadEnabled: env_1.default.kadEnabled,
            kadDbPath: env_1.default.kadDbPath,
            kadBootstrapPeers: env_1.default.kadBootstrapPeers,
    ...
    async startPeerServices() {
        if (this.cloudConfiguration.cloudType === 'public' ||
        ...
        if (!this.cloudConfiguration.kadEnabled)
            return;
        ...
        if (this.cloudConfiguration.cloudType === 'public' &&
            !this.cloudConfiguration.kadBootstrapPeers?.length) {
            console.warn("You're running a public cloud node without any kad bootstrap peers, which means you will not get any data from the network." +
                '\n\nConfigure bootstrap peers to connect to using env.ULX_BOOTSTRAP_PEERS.');
        }
        this.kad = await new kad_1.default({

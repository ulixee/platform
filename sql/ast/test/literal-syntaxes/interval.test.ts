import { checkInterval } from '../../testing/helpers';
import { intervalToString } from '../../lib/helpers/IntervalUtils';
...
describe('Interval literals', () => {
  ...
  it('produces a string', () => {
    expect(intervalToString({ years: 1 })).toEqual('1 year');
    expect(intervalToString({ years: -1 })).toEqual('-1 years');
    expect(intervalToString({ years: 2 })).toEqual('2 years');
    expect(intervalToString({ minutes: -2, seconds: 1 })).toEqual('-00:01:59');
    expect(intervalToString({ milliseconds: 1 })).toEqual('00:00:00.001');
    expect(intervalToString({ milliseconds: 10 })).toEqual('00:00:00.01');
  });
});

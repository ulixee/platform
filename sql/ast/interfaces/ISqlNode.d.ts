...
export declare type IStatement = ISelectStatement | IInsertStatement | IUpdateStatement | IDeleteStatement | ICommentStatement | IValuesStatement;
...
export declare type IFunctionArgumentMode = 'in' | 'out' | 'inout' | 'variadic';
...
export declare type IDataTypeDef = IArrayDataTypeDef | IBasicDataTypeDef;
...
export declare type ISelectStatement = ISelectFromStatement | IValuesStatement;
...
export declare type IFrom = FromTable | IFromStatement | IFromCall;
...
export declare type IJoinType = 'INNER JOIN' | 'LEFT JOIN' | 'RIGHT JOIN' | 'FULL JOIN' | 'CROSS JOIN';
export declare type IExpr = IExprRef | IExprParameter | IExprList | IExprArrayFromSelect | IExprNull | IExprExtract | IExprInteger | IExprDefault | IExprMember | IExprValueKeyword | IExprArrayIndex | IExprNumeric | IExprString | IExprCase | IExprBinary | IExprUnary | IExprCast | IExprBool | IExprCall | ISelectStatement | IExprConstant | IExprTernary | IExprOverlay | IExprSubstring;
/**
...
export declare type ILogicOperator = 'OR' | 'AND';
export declare type IEqualityOperator = 'IN' | 'NOT IN' | 'LIKE' | 'NOT LIKE' | 'ILIKE' | 'NOT ILIKE' | '=' | '!=';
export declare type IMathOpsBinary = '|' | '&' | '>>' | '^' | '#' | '<<' | '>>';
export declare type IComparisonOperator = '>' | '>=' | '<' | '<=' | '@>' | '<@' | '?' | '?|' | '?&' | '#>>' | '~' | '~*' | '!~' | '!~*';
export declare type IAdditiveOperator = '||' | '-' | '#-' | '&&' | '+';
export declare type IMultiplicativeOperator = '*' | '%' | '/';
export declare type IConstructOperator = 'AT TIME ZONE';
export declare type IBinaryOperator = ILogicOperator | IEqualityOperator | IComparisonOperator | IAdditiveOperator | IMultiplicativeOperator | IMathOpsBinary | IConstructOperator;
...
export declare type IExprLiteral = IExprConstant | IExprInteger | IExprNumeric | IExprString | IExprBool | IExprNull;
...
export declare type IUnaryOperator = '+' | '-' | 'NOT' | 'IS NULL' | 'IS NOT NULL' | 'IS TRUE' | 'IS FALSE' | 'IS NOT TRUE' | 'IS NOT FALSE';
...
export declare type IValueKeyword = 'current_catalog' | 'current_date' | 'current_role' | 'current_schema' | 'current_timestamp' | 'current_time' | 'localtimestamp' | 'localtime' | 'session_user' | 'user' | 'current_user' | 'distinct';
/**
 * Function calls.
 *
...
export declare type IGeometricLiteral = IPoint | ILine | ISegment | IBox | IPath | IPolygon | ICircle;
...
export declare type ISegment = [IPoint, IPoint];
export declare type IBox = [IPoint, IPoint];
export interface IPath {
    closed: boolean;
    path: IPoint[];
}
export declare type IPolygon = IPoint[];

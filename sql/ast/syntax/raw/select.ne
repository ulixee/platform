...
selection_statement -> select_statement {% unwrap %}
            | select_values {% unwrap %}

selection_paren -> lparen selection_statement rparen {% get(1) %}
...
select_values -> kw_values select_or_insert_values {% x => track(x, {
    type: 'values',
    values: x[1],
}) %}
...
select_or_insert_values -> select_value (comma select_value {% last %}):* {% ([head, tail]) => {
    return [head, ...(tail || [])];
} %}

select_value -> lparen select_expr_list_raw rparen {% get(1) %}


select_expr_list_raw -> expr_or_select (comma expr_or_select {% last %}):* {% ([head, tail]) => {
    return [head, ...(tail || [])];
} %}

...
expr_array -> %kw_array %lbracket expr_subarray_items:? %rbracket {% x => track(x, {
        ...
        | %kw_array lparen selection_statement rparen {% x => track(x, {
                    type: 'array select',
                    select: unwrap(x[2]),
                }) %}
...
expr_function_args -> array_of[expr_function_arg_item] {% ([x]) => x.map(unwrap) %}
expr_function_arg_item -> 
    word "=>" expr_function_arg_value {% x => unwrap({ key: x[0], ...x[2] }) %}
    | expr_function_arg_value {% x => unwrap(x) %}

expr_function_arg_value
...
expr_fn_name -> ((word %dot):?  word_or_keyword {% x => track(x, {
    ...
    | ((%kw_any | %kw_some | %kw_all | %kw_default) {% x => track(x, {
            name: toStr(unwrap(x)),
        })%})

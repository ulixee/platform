...
export class ChromeAliveManager extends EventEmitter {
  ...
  #apiByMinerAddress = new Map<
    string,
    {
      api: ChromeAliveApi<IDesktopAppApis, IDesktopAppEvents>;
      id: string;
      wsToCore: WebSocket;
  ...
  private async connectToMiner(address: string): Promise<void> {
    if (!address) return;
    const api = new ChromeAliveApi<IDesktopAppApis, IDesktopAppEvents>(
      address,
  ...
  private onChromeAliveEvent<T extends keyof IDesktopAppEvents & string>(
    minerAddress: string,
    eventType: T,

...
const CreditPaymentService_1 = require("@ulixee/datastore/payments/CreditPaymentService");
...
class PrivateDesktopApiHandler extends eventUtils_1.TypedEventEmitter {
    ...
    async createAccount(request) {
        return await this.apiManager.accountManager.createAccount(request.name, request.suri, request.password);
    }
    ...
    async transferArgonsFromMainchain(request) {
        await this.apiManager.accountManager.transferMainchainToLocal(request.address, request.milligons);
    }
    async transferArgonsToMainchain(request) {
        return this.apiManager.accountManager.transferLocalToMainchain(request.address, request.milligons);
    }
    ...
    async acceptArgonRequest(request) {
        const argonFile = IArgonFile_1.ArgonFileSchema.parse(request.argonFile);
        if (argonFile.credit) {
    ...
    async importArgons(claim) {
        const argonFile = IArgonFile_1.ArgonFileSchema.parse(claim.argonFile);
        if (argonFile.credit) {
            await this.saveCredit({ credit: argonFile.credit });
            return;
        }
    ...
    async deployDatastore(args) {
        ...
        const { compressedDbx, adminSignature, adminIdentity: identityResult, } = await apiClient.download(id, version, adminIdentity);
    ...
    async saveCredit(arg) {
        const credit = await CreditPaymentService_1.default.storeCreditFromUrl(arg.credit.datastoreUrl, arg.credit.microgons, await this.apiManager.accountManager.getDatastoreHostLookup());
    ...
    async createCredit(args) {
        ...
        const microgons = argons * Number(ArgonUtils_1.default.MicrogonsPerArgon);
        ...
        try {
            ...
            const file = {
                version: localchain_1.ARGON_FILE_VERSION,
                credit: {
            ...
            return {
                ...
                file,
                name: `₳${argons} at ${(datastore.name ?? datastore.scriptEntrypoint)?.replace(/[.\\/]/g, '-') ??
    ...
    async dragArgonsAsFile(args, context) {
        ...
        await ArgonFile_1.default.create(args.rawJson, file);
        context.startDrag({
            file,
            icon: argIconPath,
        });

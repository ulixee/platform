...
class ChromeAliveWindow {
    ...
    constructor(session, staticServer, cloudAddress) {
        ...
        const currentWindow = electron_1.BrowserWindow.getFocusedWindow() ?? electron_1.BrowserWindow.getAllWindows()[0]; // Get the currently focused window
        const windowBounds = currentWindow.getBounds();
        const display = electron_1.screen.getDisplayNearestPoint({ x: windowBounds.x, y: windowBounds.y });
    ...
    async activateView(mode) {
        ...
        if (mode === 'Live' || mode === 'Timetravel' || mode === 'Finder') {
            ...
            __classPrivateFieldGet(this, _ChromeAliveWindow_mainView, "f").bringToFront();
            ...
            if (page) {
                ...
                if (__classPrivateFieldGet(this, _ChromeAliveWindow_mainView, "f").webContents.getURL() !== url) {
                    ...
                    if (mode === 'Output') {
                        __classPrivateFieldGet(this, _ChromeAliveWindow_mainView, "f").webContents.openDevTools({ mode: 'bottom' });
    ...
    async relayout() {
        ...
        if (!__classPrivateFieldGet(this, _ChromeAliveWindow_mainView, "f").isHidden)
            __classPrivateFieldGet(this, _ChromeAliveWindow_mainView, "f").setBounds(remainingBounds);
        if (!this.activeReplayTab?.view?.isHidden)
            this.activeReplayTab.view.setBounds(remainingBounds);
    }
    closeOpenPopup() {
        try {
